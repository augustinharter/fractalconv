...
class Destiller(nn.Module):
    ...
    def propagate(self, mode="encode", stimulus=None, actis=None, n_reps=3, 
        ...
        if actis is None:
            ...
            actis = [T.zeros(batchsize, ldim, res, res) for res in self.resolutions]
        else:
            ...
            for layer_idx in range(depth)[::direction]:
                ...
                if not layer_idx:  # if first layer
                    if stimulus is not None:
                        ...
                        bottom_up = T.zeros(batchsize, width, width, ldim)
    ...
    for epoch in range(50):
        for batch_n, X in enumerate(train_loader):
            ...
            if not batch_n%args.vis:
                ...
                print(epoch, batch_n, losses, "zero%:", zeros.float().mean().numpy().round(3))
